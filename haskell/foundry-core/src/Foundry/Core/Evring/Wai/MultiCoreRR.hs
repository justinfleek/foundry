...
module Foundry.Core.Evring.Wai.MultiCoreRR
...
import Foundry.Core.Evring.Wai.Conn (ConnContext, newConnContext, startConnection)
...
runServerMultiCoreRR settings@ServerSettings {..} app = do
  ...
  -- Convert to Vector for safe O(1) indexing
  let queueVec = V.fromList queues
  _ <- forkOn 0 $ runAcceptor listenSock queueVec numCores
...
runAcceptor :: Socket -> Vector ConnQueue -> Int -> IO ()
runAcceptor sock queueVec numWorkers = do
  ...
  forever $ do
    ...
    -- Safe indexing: idx is always in [0, numWorkers-1] due to mod
    let queue = queueVec V.! idx
    success <- pushConn queue (fromIntegral fd) clientAddr
      ...
      where
        go' !hadAny = do
          ...
          case mConn of
            Nothing -> pure hadAny
            Just (fd, addr) -> do
              startConnection ctx loop (Fd (fromIntegral fd)) addr app
              go' True
...
dispatchBatch loop@Loop{..} !count !i
  ...
  | otherwise = do
      ...
      dispatchRaw loop (fromIntegral userData) (fromIntegral res)
