-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
--                                       // Foundry.Core.Coeffect.Discharge
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
--
...
--                                                              — Neuromancer
--
-- Discharge proof generation and verification. Ported from straylight-llm
-- production code. A DischargeProof provides cryptographic evidence that a
-- computation accessed only the resources it declared (its coeffects).
--
-- The proof is:
...
module Foundry.Core.Coeffect.Discharge
    ( -- * Proof Generation
      generateProof
    , generatePureProof
    , fromGatewayTracking
...
import Foundry.Core.Coeffect.Types
    ( Coeffect (Pure, Network, Auth)
    , NetworkAccess (NetworkAccess, naUrl)
    , FilesystemAccess
    , Hash (Hash, unHash)
    , PublicKey (PublicKey)
    , Signature (Signature)
    ...
    , OutputHash (OutputHash, ohHash)
    , isPure
    )
import Foundry.Core.Coeffect.Types qualified as CT
import Foundry.Core.Effects.Graded
    ( GatewayCoEffect (gceHttpAccess, gceAuthUsage)
    , GatewayProvenance (gpTimestamp)
    )
import Foundry.Core.Effects.Graded qualified as G
...
generateProof
    ...
    -> [CT.AuthUsage]                   -- ^ Auth usage evidence
    -> UTCTime                          -- ^ Start time
    -> UTCTime                          -- ^ End time
    -> DischargeProof
...
determineCoeffects coeff
    | Set.null (gceHttpAccess coeff) 
      && Set.null (gceAuthUsage coeff) = [Pure]
    | otherwise = 
...
convertHttpAccess G.HttpAccess{..} = NetworkAccess
    { naUrl = haUrl
    , CT.naMethod = haMethod
    , CT.naContentHash = Hash BS.empty  -- Would need response body
    , CT.naTimestamp = haTimestamp
    }

-- | Convert GatewayM AuthUsage to Coeffect.Types AuthUsage
convertAuthUsage :: G.AuthUsage -> CT.AuthUsage
convertAuthUsage G.AuthUsage{..} = CT.AuthUsage
    { CT.auProvider = auProvider
    , CT.auScope = Just auScope
    , CT.auTimestamp = auTimestamp
    }
...
verifyEvidenceMatches proof
  ...
  where
    ...
    isNetworkCoeffect (CT.Combined cs) = any isNetworkCoeffect cs
...
hashProofContent DischargeProof{..} =
    sha256Hash $ BS.concat
        ...
        , BS.concat [ encodeUtf8 (CT.auProvider au) | au <- dpAuthUsage ]
        ]
